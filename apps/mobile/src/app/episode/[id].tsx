...
export default function EpisodeScreen() {
  ...
  const { loadEpisode, setShowMiniPlayer, episode: currentEpisode } = usePlayer();

  // Load episode into player context and hide mini player while on this screen
  useEffect(() => {
    // Hide mini player while viewing full episode player
    setShowMiniPlayer(false);

    // Load episode if not already loaded or different episode
    if (!currentEpisode || currentEpisode.id !== mockEpisode.id) {
      loadEpisode(mockEpisode);
    }
  ...
  const handleBack = () => {
    // Show mini player when navigating back
    setShowMiniPlayer(true);
    router.back();
  };
